...
def get_all_issues(state='all'):
    """Get all issues."""
    cmd = ['gh', 'issue', 'list', '--json', 'number,title,state,labels', '--limit', '1000', '--state', state]
...
def categorize_issues(issues):
    ...
    for issue in issues:
        ...
        # Count by theme
        theme_labels = ['strategy', 'testing', 'risk-management', 'monitoring',
                       'infrastructure', 'analytics', 'code-quality', 'documentation']
...
def print_stats(stats, priority_open, ai_ready):
    ...
    if ai_ready:
        ...
        for issue in ai_ready[:5]:
            labels = [l['name'] for l in issue['labels']]
            priority = [l for l in labels if l.startswith('P')][0] if any(l.startswith('P') for l in labels) else '?'
