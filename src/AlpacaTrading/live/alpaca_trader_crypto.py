...
Supports both US Equities and Cryptocurrencies.
"""
...
class AlpacaConfig:
    ...
    Attributes:
        ...
        feed: Data feed for Stocks (default: "iex")
        crypto: Whether to trade crypto assets (default: False)
    """
    ...
    def from_env(cls) -> "AlpacaConfig":
        ...
        - ALPACA_CRYPTO (optional, default: false)
        ...
        crypto = os.getenv("ALPACA_CRYPTO", "false").lower() == "true"
        ...
        return cls(
            ...
            paper=paper,
            feed=CryptoFeed.US,  # Default stock feed
            crypto=crypto,
        )
...
class AlpacaTrader:
    ...
    Features:
    - Real-time market data streaming via WebSocket (Stocks or Crypto)
    ...
    def __init__(self, config: AlpacaConfig):
        ...
        # Initialize data stream based on asset class
    ...
    async def _handle_trade(self, trade: Trade):
        ...
        tick = MarketDataPoint(
            ...
            volume=float(
                trade.size
            ),  # Note: CryptoDataStream also uses 'size' for volume
        )
    ...
    async def _handle_quote(self, quote: Quote):
        ...
        tick = MarketDataPoint(
            timestamp=quote.timestamp, symbol=quote.symbol, price=mid_price, volume=0
        )
    ...
    def start_streaming(
        ...
        # Method names are consistent across StockDataStream and CryptoDataStream
        if data_type == "trades":
    ...
    def __repr__(self) -> str:
        ...
        asset = "CRYPTO" if self.config.crypto else "STOCK"
        streaming = "STREAMING" if self.is_streaming else "IDLE"
        return f"AlpacaTrader(mode={mode}, asset={asset}, status={streaming}, symbols={len(self.subscribed_symbols)})"
